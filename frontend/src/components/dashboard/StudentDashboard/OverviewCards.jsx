  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
      <StatCard
        ...
        color="yellow"
        trend={stats?.xp_trend > 0 ? 'up' : 'down'}
        trendValue={`${Math.abs(stats?.xp_trend || 0)}% this week`}
      />
      ...
      <StatCard
        ...
        color="green"
        suffix="%"
        trend={stats?.completion_trend > 0 ? 'up' : stats?.completion_trend < 0 ? 'down' : null}
        trendValue={stats?.completion_trend ? `${Math.abs(stats.completion_trend)}% vs last month` : null}
      />
    </div>
  )
}
